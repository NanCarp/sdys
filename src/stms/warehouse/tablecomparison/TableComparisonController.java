...
/**
 * @ClassName: TableComparisonController
 * @Description: 库位对比
 * @author: LiYu
...
public class TableComparisonController extends Controller {
    // 数据列表
    public void index() {
        render("table_comparison.html");
    }
    ...
    public void getRecord() {
        ...
        if (id != null) {//编辑
            Record record = Db.findById("t_table_comparison", id);
            setAttr("record", record);
        }

        render("table_comparison_detail.html");
    }
    ...
    public void save() {
        ...
        if (id == null && TableComparisonService.isDuplicate(batchNo, trayNo)) {
        ...
        // 检测是否有后续业务单据
        if (id != null && TableComparisonService.hasOtherBusiness(batchNo, trayNo)) {
            response.put("isSuccess", false);
    ...
    public void delete(){
        ...
        for (String id : ids) {
            hasOtherBusiness = TableComparisonService.hasOtherBusiness(id);
            if (hasOtherBusiness) {
                response.put("isSuccess", false);
        ...
        boolean result = TableComparisonService.delete(ids);
        response.put("isSuccess", result);
    ...
    public void importUI() {
        // 导入页面
        render("table_comparison_import.html");
    }
    ...
    */
    public void importByExcel() {
        ...
        Map<String, Object> msgMap = TableComparisonService.importByExcel(uploadFile, getSession());
